...
def read_data(corpus_type, fn, column_no=-1, delimiter =' '):
	...
	for k in range(len(lines)):
		...
		# strings = line.split(delimiter)	# *原code
		strings = line.split()	# *modify
...
if __name__ == '__main__':
	...
	suffixs = ['train', 'dev', 'test', 'unlabel']		# train.txt(src_lang), dev.txt(src_lang), test.txt(tgt_lang), unlabel.txt(tgt_lang training data) → spanner.train, spanner.dev, …
